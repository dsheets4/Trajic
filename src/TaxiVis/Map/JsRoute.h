...
namespace Map
{
   ...
   //! Smart pointer definition used for passing around JsRoute pointers.
   typedef SHARED_POINTER<JsRoute> JsRoutePtr;


   //! Wraps the DataAccess Route so that it can be exposed to JavaScript.
   class JsRoute : public MapObject
   {
      Q_OBJECT
   ...
   public:
      ...
      // ----------------------------------------------------------------------
      JsRoute(QObject *pParent);
      JsRoute(DataAccess::RoutePtr pRoute, QObject *pParent);
      ~JsRoute();
